...
if(isset($prodid)&&(!empty($prodid))){
    $cathegorie=getCathegoryByID($prodid);
}
if(isset($prodid)&&(!empty($prodid))){
    $souscathegorie=getSOUSCathegByIDCat($prodid);
}

$famille=getAllFamilles();

if(isset($prodid)&&(!empty($prodid))){
    $produit=getProduitByIDCath($prodid);
 }
 ...
 //3-les nombres de produits dans la base
  $page=filter_var($_GET['page'],FILTER_SANITIZE_NUMBER_INT);
 $nbrresults=$conn->prepare("SELECT * FROM  produit INNER JOIN categorie ON produit.id_cat=categorie.id_cat WHERE produit.id_cat=$prodid ");
 $nbrresults->execute();
...
$resultspg=$conn->prepare("SELECT * FROM  produit INNER JOIN categorie ON produit.id_cat=categorie.id_cat WHERE produit.id_cat=$prodid LIMIT ".$results."  OFFSET  ".($page-1)* $results);
$resultspg->execute();
}else{
 echo "ID INVALIDE !!";
}
...
<!Doctype html>
	...
	<head>
		...
		<title>CATEGORIE<?php echo $cathegorie['id_cat'] ; ?> </title>
...
<section id="<?php echo $cathegorie['idclassc'] ; ?>">
    <h1 >#<?php echo $cathegorie['nom_cat'] ; ?></h1>
    <p> <br></p>
    <h5</h5> <br>
    <br><br>
    </section>
 ...
 ?>
   ...
   <?php echo $cathegorie['nom_cat'] ; ?><?php echo "</a>"; ?>
...
foreach($souscathegorie as $index=>$souscat){?>
<div class="sc">
  <div class="col-sm-6">
    <div class="card">
      <div class="card-body">
      <h3 class="card-title"><?php echo $souscat['nomscathg']; ?></h3>
        <p class="card-text"></p>
        <a href="souscat.php?id=<?php echo $souscat['idscatheg']; ?>&&page=1"  type="submit"class="btn btn-success">EXPLORER</a>
      </div>
    </div>
  </div>
</div>
...
<div class="pge">
 <nav aria-label="Page navigation example">
  ...
  if($page>1){echo"  <li class='page-item '>
    <a class='page-link' href='cathegory.php?id=".$prodid."&&page=".($page-1)."'>Precidant</a></li>";}else{
  ...
  <?php  //creation des pages de pagination
   ...
   for($count=1;$count<=$totalpages;$count++){
     //-8 la page active
if($page==$count){
  echo "   <li class='page-item active'><a class='page-link' href='cathegory.php?id=".$prodid."&&page=".$prodid."&&page=".$count."'>".$count."</a></li>
";
}else{
echo "  
<li class='page-item'><a class='page-link' href='cathegory.php?id=".$prodid."&&page=".$count."'>".$count."</a></li> 
";
 
 }  }    ?><?php echo" <li class='page-item'>
 <a class='page-link' href='cathegory.php?id=".$prodid."&&page=".($page+1)."'>Suivant</a>
</li>";      ?>
 </ul>
 </nav>
 </div>
